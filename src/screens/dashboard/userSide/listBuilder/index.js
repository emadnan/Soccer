...
import BuilderGrid from '../../../../components/builderGrid/index';
...
export default function AlertBuilder() {
  ...
  const AddAlert = () => {
    ...
    const createAlertAPI = async () => {
      setVisible(!visible);
      const url = 'https://api.footballstatspro.com/api/create_listBuilder';
  ...
  const fetchAlertDataById = async id => {
    ...
    if (token) {
      try {
        ...
        console.error('Error fetching match data in List:', error);
      }
    }
  };
      ...
      <View
        ...
        <ScrollView nestedScrollEnabled={true}>
          <View style={styles.titleView}>
            <View style={{width: wp(5)}} />
            <Text style={styles.welcomeText}>List Builder</Text>
            <Ionicons
              onPress={() => setVisible(!visible)}
              color={Colors.black}
